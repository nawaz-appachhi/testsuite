...
public class TestNGCustomReportListener implements IReporter{
	...
	private String reportTitle= "Myntra Store Front Automation Report";
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (!testname.equalsIgnoreCase(lastClassName)) {
					if (mq > 0) {
						cq += 1;
	...
	private void startResultSummaryTable(String style) {
		...
		writer.println("<tr><th>Class</th>"
				+ "<th>Method</th><th>Exception Info</th><th>Execution Time<br/>(hh:mm:ss)</th></tr>");
		m_row = 0;
	}
...
//		writer.print(Utils.stackTrace(exception, true)[0]);
	...
	//	Collections.sort(allMethods, new TestMethodSorter());
		...
		if (result != null) {
			int s = result.size();
			HashMap<Integer, ITestNGMethod> hm = new HashMap<Integer, ITestNGMethod>();
			for (ITestNGMethod m : result) {
				hm.put(m.getPriority(), m);
			}
			result.clear();
			// List<ITestNGMethod> result1 = new ArrayList<ITestNGMethod>() ;
//			for (int i = 1; i <= s; i++) {
//				result.add(hm.get(i));
//			}
...
//		        it.remove(); // avoids a ConcurrentModificationException
	...
	private void tableStart(String cssclass, String id) {
		writer.println("<table table-layout:fixed;  style=white-space: pre-line cellspacing=0 cellpadding=0"
	...
	protected void endHtml(PrintWriter out) {
		out.println("<center> End of the Report </center>");
