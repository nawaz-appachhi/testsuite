...
public class RuleEngineDP_global_GST {

/** The log. */

	static org.slf4j.Logger log = LoggerFactory.getLogger(RuleEngineDP_global_GST.class);
/** The list. */
...
/** The date. */
	...
	String initializationSpreadSheet[]={"myntraservice@crypto-oath-179310.iam.gserviceaccount.com",
			"GST Global Tax Master Configuration 1","TaxMaster - 1","GST Global Tax Master Configuration 2","Tax Master"};
	private static final String CLIENT_ID = "myntraservice@crypto-oath-179310.iam.gserviceaccount.com";
	private static final String filePath = System.getProperty("user.dir")+"/Myntra-taxmaster.p12";
	private static final String spreadsheeetTitle[]={"GST Global Tax Master Configuration 1","GST Global Tax Master Configuration 2"};
	private static final String worksheetName[]={"TaxMaster - 1","Tax Master"};
...
*/
	...
	public static Object[][] payload(ITestContext testContext) throws IOException, GeneralSecurityException, ServiceException, URISyntaxException {
		...
		List<ListEntry> listfeed2 = sheetAPI.testConnectToSpreadSheet(CLIENT_ID,filePath,spreadsheeetTitle[0],worksheetName[0]);
		List<ListEntry> listfeed1 = sheetAPI.testConnectToSpreadSheet(CLIENT_ID,filePath,spreadsheeetTitle[1],worksheetName[1]);
		List<ListEntry> listfeed = new ArrayList<>(listfeed1.size() + listfeed2.size());
		listfeed.addAll(listfeed1);
		listfeed.addAll(listfeed2);
		log.debug("No of rows - "+listfeed.size());
			...
			for(String keys:cec.getTags()){
				...
				switch (key_name) {
					...
					case "cess":
					{
						...
						cvr.setDate(date);
						cvr.setMrpValue(random);
						list.add(new Object[] { createNewObject(cvr), IGSTValueDouble,SGSTValueDouble,CGSTValueDouble,CESSValueDouble,taxType });
